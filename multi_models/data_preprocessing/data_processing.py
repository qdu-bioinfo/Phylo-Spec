...
def save_node_features_with_pickle(node_features, node_relations, node_weights, train_group, output_file):
    ...
    for node, feature_list in node_features.items():
        ...
        for idx, feature in enumerate(feature_list):
            print(f"Node: {node}, Batch {idx}, Feature Shape: {feature.shape}")
            print("----------------------------------------")

        concatenated_features = np.concatenate(feature_list, axis=0)
        node_features_dict[node] = concatenated_features
        print("----------------------------------------")
        print(f"Node: {node}, Concatenated Feature Shape: {concatenated_features.shape}")

    branch_lengths = {node: 1 - weight for node, weight in node_weights.items()}
    ...
    # print(f"Node features and labels saved to {output_file}")
...
def process_unclassified_features(tree, abundance_table, taxonomy_path):
    ...
    for feature in unclassified_features:
        matched_taxa = None
        missing_otus = pd.Series(dtype='str')
        ...
        for level in taxonomic_levels:
            ...
            if not matched_taxa.empty:
                ...
                if not missing_otus.empty:
                    break

        if missing_otus.empty:
            print(f"No missing OTUs found in abundance table for feature '{feature}'.")
            continue

        unclassified_abundance = abundance_table[feature]
        average_abundance = unclassified_abundance / len(missing_otus)

        for otu in missing_otus:
            if otu in table.columns:

                table[otu] += average_abundance
            else:
