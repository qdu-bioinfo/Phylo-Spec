...
# Calculate the optimal information gain
def best_information_gain(X, y, feature_idx, parent_entropy):
    ...
    for thresh in thresholds:
        ...
        if len(y_left) > 0 and len(y_right) > 0:
            weighted_entropy = (len(y_left) / len(sorted_y)) * entropy(y_left) + (len(y_right) / len(sorted_y)) * entropy(y_right)
...
# Calculate the feature information gain of each node
...
def calculate_node_importance_recursive(node, parent_importance, feature_importances, node_relations, branch_lengths, importance_scores):
    ...
    if node in node_relations:
        for child in node_relations[node]:
            ...
            calculate_node_importance_recursive(child, current_importance * (1 - dist), feature_importances, node_relations, branch_lengths, importance_scores)

# Calculate the importance of all nodes
